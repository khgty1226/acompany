...
public class NextITFileUpload {
	...
	public List<AttachVO> fileUpload(
		...
		System.out.println("NextITFileUpload fileupload boFiles.length : " + boFiles.length);
		...
		if(boFiles.length > 0) {
			for(MultipartFile boFile : boFiles) {
				if(!boFile.isEmpty()) {
					...
					System.out.println("NextITFileUpload fileupload attachList : " + attachList);
				}
			}
		}
		return attachList;
	...
	private AttachVO getAttachInfoAndFileUpload(MultipartFile boFile, String category, String path) throws IllegalStateException, IOException {
		...
		//리눅스
		//String filePath = "/home/pc14/upload/" + path;
		//윈도우
		//String filePath = "C:/pc14/upload/"+ path;
