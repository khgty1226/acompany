...
function fn_boardViewCode(boardCode){
	...
	location.href="${pageContext.request.contextPath}/mySaveList?boardCode="+bc+"&memNo="+mn+"&searchType="+st+"&searchWord="+sw+"&rowSizePerPage="+rpp;
...
$(function(){
	...
	$('#id_btn_reset').click(function() {
		...
		location.href="${pageContext.request.contextPath}/mySaveList";
	});
		...
		<div class="div_table">
			<div class="col-xl-12">
				<table class="table">
					...
					<c:forEach items="${mySaveList }" var="mySaveList">
						<tr class="boardTr">
							<%-- <td>${categoryList.boardNo }</td> --%>
							<td>
								<p class="year">${fn:substringBefore(mySaveList.boardDate, '.')}</p> <!-- "2023" 출력 -->
							    <p class="md" style="margin-bottom:0px;">${fn:substringAfter(mySaveList.boardDate, '.')}</p>  <!-- "07/21" 출력 -->
							</td>
							<td><a href="${pageContext.request.contextPath }/boardDetailView?boardNo=${mySaveList.boardNo }">${mySaveList.boardTitle }</a></td>
							<td>${mySaveList.boardCategory }</td>
							<%-- <td>${board.boardTitle }</td> --%>
							<td>${mySaveList.memNo }</td>
							<td>${mySaveList.boardHit }</td>
							<td>${mySaveList.boardLike }</td>
						</tr>
					</c:forEach>
					</tbody>
				</table>
