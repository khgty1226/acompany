...
<script>
	...
	$(function(){
		...
		if(check == 'apply'){
			...
			alert("신청에 실패했습니다. 전산실에 문의해주세요. 010-7358-4002");
			...
			alert("카풀의 탑승 가능 좌석이 부족합니다. 다른 카풀을 이용해주세요.");
		}
...
<body>
    ...
    <div class="container-xxl project py-5">
        <div class="container">
            <div class="text-center mx-auto mb-5 wow fadeInUp" data-wow-delay="0.1s" style="max-width: 600px;">
                ...
                <div>
						<c:if test="${member.roleCode ne 'AD' }">
							<button type="button" style="width: 200px" id="myCarPoolView" class="btn btn-primary mybtn" onclick="location.href='${pageContext.request.contextPath }/myCarPoolView'">나의 카풀</button>
						...
						<c:if test="${member.roleCode eq 'AD' }">
							<button type="button" style="width: 200px;" id="carPoolAdminView" class="btn btn-primary mybtn" onclick="location.href='${pageContext.request.contextPath }/carPoolAdminView'">카풀 관리</button>
						</c:if>
                </div>
            </div>
            <div class="row g-4 wow fadeInUp" style="position:relative" data-wow-delay="0.3s">
            
            <div id="map" style="width:100%; height:540px; position:absolute; z-index:1"></div>
	...
	$(function(){
		...
		geocoder.addressSearch(address, function(result, status) {
		    ...
		    // 정상적으로 검색이 완료됐으면 
		     if (status === kakao.maps.services.Status.OK) {
				...
				boundss.extend(markerD.getPosition());
		        ...
		        map.setCenter(coords);
		     	...
		     	// 마커에 클릭이벤트를 등록합니다
		        kakao.maps.event.addListener(markerD, 'click', function() {
		              // 마커 위에 인포윈도우를 표시합니다
		              map.panTo(markerD.getPosition());
		        });
		...
		$("#carPoolSearchBtn").on('click', function(){
			...
			let cpGoOut = $("#cpGoOut").val();
	
			let listDiv = $("#carPoolList");
			let carPoolList = $("#carPoolList").children().toArray();
					
			for(let i = 0 ; i < carPoolList.length; i++){
				console.log("carPoolList.data(goout)=", $(carPoolList[i]).data('goout'));
				if($(carPoolList[i]).data('goout') == cpGoOut){
