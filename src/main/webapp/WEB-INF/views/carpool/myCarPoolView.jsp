...
if("${login.memNo }" == null || "${login.memNo}" == ''){
	...
	$(function(){
		...
		if(check == 'cSuccess'){
			...
			alert("변경에 실패했습니다. 전산실에 문의해주세요. 010-7358-4002");
		}else if(check == 'cancel'){
			alert("카풀을 취소했습니다.");
		}else if(check == 'cfail'){
			alert("카풀 취소에 실패했습니다. 전산실에 문의해주세요. 010-7358-4002");
...
<body>
        ...
        <div class="container py-5">
            ...
            <nav aria-label="breadcrumb animated slideInDown">
                <ol class="breadcrumb text-uppercase mb-0">
                    ...
                    <li class="breadcrumb-item text-primary active" aria-current="page">나의 카풀</li>
                </ol>
            </nav>
        </div>
    </div>
        ...
        <div class="container">
            <div class="text-center mx-auto mb-5 wow fadeInUp" data-wow-delay="0.1s" style="max-width: 600px;">
                ...
                <div>
						...
						<button type="button" style="width: 200px; margin-left: 10px;" id="carPoolCreate" class="btn btn-primary mybtn" onclick="location.href='${pageContext.request.contextPath }/carPoolCreateView'">카풀 등록하기</button>
                </div>
            </div>
            ...
            <div class="col-lg-3" style="padding-left: 0; position:relative; left:15px; top:15px; z-index:2">
	            	<div class="w-100" style="padding : 5px; background-color: white; border:0.5px solid lightgray; border-bottom:none; opacity: 0.8">
						...
						<div id="carPoolGoOutDiv" class="card-body-div d-flex justify-content-center" style="margin-top:5px;margin-bottom:0;">
											 	 <button type="button" id="carPoolPassengerBtn" class="btn btn-primary mybtn goOutBtn" disabled>Passenger</button>
											 	 <button type="button" id="carPoolDriverBtn" class="btn btn-outline-primary mybtn goOutBtn">Driver</button>
						</div>
										  ...
										  <div class="card-body" style="height: 100%; padding-top:0px">
											  <div class="card-body-div hidden-form">
											    ...
											    <form id="applyForm" method="post" action="">
											    	...
											    	<input type="hidden" id="cpDriver" name="driver" value=""><br>
	        ...
	        <div class="row g-4 wow fadeInUp" style="position:relative;" data-wow-delay="0.3s">
		        <div class="col-lg-12">
		        	...
		        	<div id="searchBar" style="padding-left:0px; padding-right:0px; color:black; font-size:20px; margin-bottom: 10px">
			        	<form name="search" action="${pageContext.request.contextPath }/myCarPoolView" method="get">
								      ...
								      <td>
								      		<c:choose>
								      			<c:when test="${carPool.cpState eq 'P' }">
									      			<c:if test="${carPool.memNo eq member.memNo}">
														<select class="cpStateSelect">
															<option value="P" selected>운행중</option>
															<c:if test="${carPool.cpTakePass > 1 }">
																<option value="C">운행완료</option>
															</c:if>
															<option value="F">운행실패</option>
														</select>
														<button type="button" class="cpStateChangeBtn">변경</button>
											      	</c:if>
											      	<c:if test="${carPool.memNo ne member.memNo}">
											      		운행중
											      	</c:if>
								      			</c:when>
								      			<c:when test="${carPool.cpState eq 'C' }">
								      				...
								      				<c:if test="${carPool.memNo ne member.memNo}">
								      					<c:if test="${carPool.cpStatePass eq 'P' }">
									      					<select class="cpStateSelectPass">
												      			<option value="P" selected>완료대기</option>
												      			<option value="S">운행완료</option>
												      			<option value="F">운행실패</option>
												      		</select>
												      		<input type="hidden" name="driver" value="${carPool.memNo }">
													      	<button type="button" class="cpStateChangePassBtn">변경</button>
								      					</c:if>
								      					<c:if test="${carPool.cpStatePass eq 'S' }">
								      						완료대기
								      					</c:if>
								      				</c:if>
								      			</c:when>
	...
	$(function(){
		...
		// 마커의 이미지정보를 가지고 있는 마커이미지를 생성합니다
	     ...
	     // 마커에 클릭이벤트를 등록합니다
	        kakao.maps.event.addListener(marker, 'click', function() {
	     	...
	     	/* PassDriver 카풀, 마커 숨기기*/
	     	if($(this).find("#cpMemNo").html() == "${member.memNo}"){
		...
		$("#carPoolDeleteBtn").on('click', function(){
			if (!confirm("해당 카풀을 삭제하시겠습니까?")) {
		       ...
		       let location = applyForm.find("input[name='location']");
		       location.val("my");
				applyForm.attr("action", "${pageContext.request.contextPath }/carPoolDelete");
				applyForm.submit();
		    }
		});
		...
		let passBtn = $("#carPoolPassengerBtn");
		let driverBtn = $("#carPoolDriverBtn");


		$("#carPoolPassengerBtn").on('click', function(){
			bounds = new kakao.maps.LatLngBounds();
			
			passBtn.removeClass("btn-outline-primary");
			passBtn.addClass("btn-primary");
			
			driverBtn.removeClass("btn-primary");
			driverBtn.addClass("btn-outline-primary");
			
			passBtn.attr('disabled', true);
			driverBtn.attr('disabled', false);
			...
			// 127.40775961305604  36.32662677028527
			/* var cc = new kakao.maps.LatLng("36.32662677028527", "127.40775961305604");
			console.log("cc:", cc);
			bounds.extend(cc); */
	...
	$("#carPoolDriverBtn").on('click', function(){
			bounds = new kakao.maps.LatLngBounds();
			
			driverBtn.removeClass("btn-outline-primary");
			driverBtn.addClass("btn-primary");

			passBtn.removeClass("btn-primary");
			passBtn.addClass("btn-outline-primary");
			
			driverBtn.attr('disabled', true);
			...
			console.log("markerC:", markerC.getPosition());
	...
	$(".cpStateChangePassBtn").on("click", function(){
		...
		let cpState = $(this).parent().children('select').val();
		let driver = $(this).parent().find("input[name='driver']").val();
		if(cpState == 'P'){
			alert("'운행완료' 혹은 '운행실패'를 선택해주세요.");
			return;
		}
		...
		form.find("input[name='cpState']").val(cpState);
		form.find("input[name='memNo']").val(memNo);
		form.find("input[name='driver']").val(driver);
